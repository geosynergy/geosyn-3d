self.addEventListener('message', function (e) {
  ...
  var tileSize = e[2]
  var s = e[3]
  var isSubsurface = e[4] === 'subsurface'

  if (isSubsurface) {
    parseCoalDepth(t, tileSize, s, e[4])
    return
  }
...
function parseRaw (t, tileSize, si, tt) {
  var s = si
  var pixelSize = tileSize / s

  var url = '/api/gs/wms?VERSION=1.1.1&FORMAT=application/bil32&REQUEST=GetMap&SRS=EPSG:28356&WIDTH='+s+'&HEIGHT='+s+'&TRANSPARENT=true'+
    '&BBOX='+([t.x, t.y, t.x + tileSize, t.y + tileSize].join(','))+'&LAYERS=v4:sb_south_dem_4283'
  // console.log(url)
  var oReq = new XMLHttpRequest();
  oReq.open("GET", url, true);
  oReq.responseType = "arraybuffer";
